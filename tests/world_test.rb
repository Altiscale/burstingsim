...
class WorldTest < Test::Unit::TestCase
  ...
  def test_counters_free_hosts_multiple_expired_requested
    assert_nothing_raised do
      ...
      set_expected_count_values([3, 0, 3, 0, 1, 1, 1, 0])
  ...
  def test_counters_free_hosts_requested
    assert_nothing_raised do
      ...
      set_expected_count_values([0, 0, 0, 0, 1, 1, 1, 0])
      get_sim_count_values('sc1', 'yellowhammer')
      assert_count_values
    end
  end
  ...
  def test_counters_free_hosts_multiple_expired_other_requested
    assert_nothing_raised do
      ...
      set_expected_count_values([2, 2, 2, 2, 0, 1, 0, 2])
      get_sim_count_values('sc1', 'yellowhammer')
      assert_count_values
    end
  end
  ...
  def assert_count_values
    ...
    "cluster busy hosts count incorrect! Expecting: #{@found_cluster_busy_hosts_count} \
	   Found: #{@exp_cluster_busy_hosts_count}")
